    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      <div>
        ...
        {roomsDummyData.map((room)=>(
            <div key={room._id} className='flex flex-col md:flex-row items-start py-0 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
               <img onClick={()=> {navigate(`/rooms/${room._id}`), scrollTo(0,0)}}
                src={room?.images?.[0] }
                alt="hotel-img" title='View Room Details' className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
               <div className='md:w-1/2 flex flex-col gap-2'>
                ...
                <div className='flex flex-wrap items-center mt-3 md-6 gap-4'>
                   {room.amenities.map((item)=>(
                       <div key={`${room._id}-${item}`} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
      ...
      <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
          ...
          <div className={`flex items-center justify-between px-5 pt-2.5 min-lg:border-b border-gray-300 ${openFilters && "border-b"}`}>
